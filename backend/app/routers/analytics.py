...
async def signal_accuracy(
    ...
    """
    Model-level signal accuracy — covers ALL signals generated by the engine,
    not just user-tracked bets. Measures how often each market prediction was correct
    across settled fixtures (FT/AET/PEN with a recorded score).

    Returns hit rates segmented by market, confidence tier, and league tier.
    No auth required — this is the public transparency/trust panel.
    """
    ...
    q = (
        ...
        .where(Fixture.event_date >= from_date)
        .where(Fixture.status.in_(list(_FINAL)))
        .where(Fixture.home_score.is_not(None))
        .where(Fixture.away_score.is_not(None))
    )
    rows = (await db.execute(q)).all()
    ...
    for market, confidence, tier, hs, aws in rows:
        fn = SCORE_SETTLEABLE_MARKETS.get(market)
        if fn is None:
            continue
        try:
            won = bool(fn(int(hs), int(aws)))
        except Exception:
            continue
        ...
        s = by_market.setdefault(market, {"total": 0, "hit": 0})
        s["total"] += 1
        if won:
            s["hit"] += 1

        conf = confidence or "Low"
        c = by_confidence.setdefault(conf, {"total": 0, "hit": 0})
        c["total"] += 1
        if won:
            c["hit"] += 1

        t = tier or 3
        tl = by_tier.setdefault(t, {"total": 0, "hit": 0})
        tl["total"] += 1
        if won:
            tl["hit"] += 1

    def _pct(d):
        return round(d["hit"] / d["total"] * 100, 1) if d["total"] else None
    ...
    return {
        ...
        "total": total,
        "hit": overall["hit"],
        "hit_rate": _pct(overall),
        "by_market": sorted(
            [
                {"market": m, "total": s["total"], "hit": s["hit"], "hit_rate": _pct(s)}
        ...
        "by_confidence": [
            {"confidence": c, "total": s["total"], "hit": s["hit"], "hit_rate": _pct(s)}
            for c, s in sorted(
                by_confidence.items(),
                key=lambda kv: {"High": 0, "Medium": 1, "Low": 2}.get(kv[0], 3),
            )
        ],
        "by_league_tier": [
            {"tier": t, "total": s["total"], "hit": s["hit"], "hit_rate": _pct(s)}
