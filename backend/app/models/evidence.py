...
class ProviderObservation(Base):
    ...
    request_scope: Mapped[str | None] = mapped_column(String(255), nullable=True)
...
class MarketDefinition(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "canonical_key", "version", name="uq_market_definition_version"
        ),
    )
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
...
class OddsQuote(Base):
    ...
    __table_args__ = (
        Index(
            "ix_odds_quotes_fixture_market_time",
            "fixture_id",
            "market_key",
            "received_at",
        ),
        ...
        UniqueConstraint(
            ...
            "bookmaker",
            "selection_name",
            name="uq_odds_quote_observation_key",
        ),
    )
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    ...
    market_version: Mapped[str] = mapped_column(
    ...
    provider_observation_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("provider_observations.id"), nullable=True, index=True
    )
    availability: Mapped[str] = mapped_column(
        String(20), nullable=False, server_default="prematch"
    )
    evidence_class: Mapped[str] = mapped_column(
        String(40), nullable=False, server_default="provider"
    )
...
class FixtureRevision(Base):
    ...
    external_fixture_id: Mapped[int | None] = mapped_column(
    ...
    supersedes_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("fixture_revisions.id"), nullable=True
    )
    evidence_class: Mapped[str] = mapped_column(
        String(40), nullable=False, server_default="provider"
    )
    legacy_fixture_id: Mapped[int | None] = mapped_column(Integer, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
...
class ModelVersion(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "name", "version", "config_sha256", name="uq_model_version_identity"
        ),
    ...
    training_manifest_json: Mapped[str | None] = mapped_column(Text, nullable=True)
    max_input_available_at: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True
    )
...
class FeatureSnapshot(Base):
    ...
    fixture_revision_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("fixture_revisions.id"), nullable=False, index=True
    )
    ...
    input_refs_json: Mapped[str] = mapped_column(
        Text, nullable=False, server_default=text("'{}'")
    )
